...
@section('title','TEAM 7 || PRODUCT DETAIL')
                    ...
                    <div class="col-lg-6 col-12">
                        <div class="product-gallery">
                            <div class="flexslider-thumbnails">
                                ...
                                @php
                                    $rawPhotos = $product_detail->getRawOriginal('photo');
                                    $photos = array_filter(array_map('trim', explode(',', $rawPhotos ?? '')));
                                @endphp
                                <ul class="slides">
                                    @if(!empty($photos))
                                        @foreach($photos as $photo)
                                            @php
                                                $photoUrl = \Illuminate\Support\Str::startsWith($photo, ['http://', 'https://']) ? $photo : asset($photo);
                                            @endphp
                                            <li data-thumb="{{ $photoUrl }}">
                        ...
                        <div class="product-des">
                            <div class="short">
                                ...
                                <div class="rating-main">
                                    <ul class="rating">
                                        @php $rate = ceil($product_detail->getReview->avg('rate')) ?? 0; @endphp
                                        @for($i=1; $i<=5; $i++)
                                            @if($rate >= $i)
                            ...
                            @if($product_detail->size)
                                @php $sizes = is_array($product_detail->size) ? $product_detail->size : explode(',', $product_detail->size ?? ''); @endphp
                                <div class="size mt-4">
                                    <h4>Size</h4>
                                    <ul>
                                                        ...
                                                        @foreach($product_detail->getReview as $data)
                                                            <div class="single-rating">
                                                                <div class="rating-author">
                                                                    <img src="{{$data->user_info->photo}}" alt="User avatar">
                                                                </div>
        ...
        <div class="row">
            <div class="col-12">
                <div class="owl-carousel popular-slider">
                    @if(!empty($product_detail->rel_prods))
                            ...
                            @if($data->id !== $product_detail->id)
                                <div class="single-product">
                                    <div class="product-img">
                                        ...
                                        @if(!empty($data->photo))
                                            <img class="default-img" src="{{ asset($data->photo) }}" alt="{{ $data->title }}">
                                            <img class="hover-img" src="{{ asset($data->photo) }}" alt="{{ $data->title }}">
                                        @else
                                            <img class="default-img" src="{{ asset('images/no-image.png') }}" alt="No image">
                                        @endif
...
.star-rating { font-size: 0; padding: 0 10px; }
.star-rating__wrap { display: inline-block; font-size: 1rem; }
.star-rating__wrap:after { content: ""; display: table; clear: both; }
.star-rating__ico { float: right; padding-left: 2px; cursor: pointer; color: #F7941D; font-size: 16px; margin-top: 5px; }
